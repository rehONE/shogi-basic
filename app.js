    ...
    shogiPieces.forEach(piece => {
        html += `
            <div class="piece">
                ...
                <img src="${piece.image}" alt="${piece.name} ход" style="max-width:200px; display:block; margin:10px 0;">
...
function showRules() {
    content.innerHTML = `
        ...
        <ul>
            ...
            <li>Фигуры могут превращаться при достижении последней трети доски (зоны продвижения противника).</li>
            <li>Превращение меняет ход фигуры на более сильный (например, пешка становится золотым генералом).</li>
            <li>Не все фигуры могут превращаться: король и золотой генерал не превращаются.</li>
            <li>Игрок выбирает, превращать фигуру или нет, при её входе в зону продвижения.</li>
        </ul>
    `;
}
